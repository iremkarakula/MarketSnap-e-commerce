...
function ProductComponent({ product }) {
    ...
    const { register, handleSubmit } = useForm({ resolver: zodResolver(sizeSchema) });
    ...
    return (
        <div className='m-2 sm:m-4 space-y-4'>
            <form className='space-y-6' onSubmit={handleSubmit(onSubmit)}>
                <div className='group relative text-center'>
                   ...
                   group-hover:brightness-75 transition-brightness duration-200 ' />
                    ...
                    {product.isDiscount && <div className='absolute top-[3%] bg-red-500 w-fit py-1 px-2 rounded-r text-white font-semibold text-xs md:text-base' >%{product.discountRate}</div>}
                    ...
                    {
                        showSize && <div className="flex space-x-2 sm:space-x-4 justify-center absolute bottom-0 bg-white w-full  
                        py-6 sm:p-6 overflow-hidden">
                            {["S", "M", "L"].map((size) => {
                                return <label key={size}>
                                    <input type='radio' value={size} {...register("size")} className='peer sr-only' />
                                    <div className='w-8 h-8 sm:w-12 sm:h-12 rounded-full border-2 border-gray-300 flex justify-center items-center text-gray-700 hover:bg-accent hover:text-accent-foreground peer-checked:border-primary peer-checked:bg-primary peer-checked:text-primary-foreground'>
                                        {size}
                                    </div>
                                </label>
